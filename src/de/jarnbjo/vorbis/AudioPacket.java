...
class AudioPacket {
	...
	private int windowCenter, leftWindowStart, leftWindowEnd, leftN,
	...
	protected AudioPacket(final VorbisStream vorbis, final BitInputStream source)
		...
		for (int i = 0; i < channels; i++) {
			...
			Floor decodedFloor = sHeader.getFloors()[floorNumber].decodeFloor(
					vorbis, source);
		...
		for (int i = 0; i < mapping.getSubmaps(); i++) {
			...
			residue.decodeResidue(vorbis, source, mode, ch, doNotDecodeFlags,
					pcm);
		}
		...
		for (int i = 0; i < channels; i++) {
			MdctFloat mdct = blockFlag ? iHeader.getMdct1() : iHeader
					.getMdct0();
