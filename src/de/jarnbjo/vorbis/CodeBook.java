...
class CodeBook {
	...
	protected CodeBook(BitInputStream source) throws VorbisFormatException,
			IOException {
		...
		if (ordered) {
			...
			for (int i = 0; i < entryLengths.length;) {
				...
				if (i + num > entryLengths.length) {
					throw new VorbisFormatException(
							"The codebook entry length list is longer than the actual number of entry lengths.");
				}
			...
			if (codeBookLookupType == 1) {
				for (int i = 0; i < entries; i++) {
					...
					for (int j = 0; j < dimensions; j++) {
						int multiplicandOffset = (i / indexDivisor)
								% codeBookLookupValues;
						valueVector[i][j] = codeBookMultiplicands[multiplicandOffset]
								* codeBookDeltaValue
								+ codeBookMinimumValue
								+ last;
		...
		default:
			throw new VorbisFormatException(
					"Unsupported codebook lookup type: " + codeBookLookupType);
		}
	}
...
//	private static long totalTime = 0;
	...
	protected int readInt(final BitInputStream source) throws IOException {
		...
		/*
		 * HuffmanNode node; for(node=huffmanRoot; node.value==null;
		 * node=source.getBit()?node.o1:node.o0); return node.value.intValue();
		 */
	}

	protected void readVvAdd(float[][] a, BitInputStream source, int offset,
			int length) throws VorbisFormatException, IOException {
	 ...
	 * else{ for(i=0;i<length;){ entry=readInt(source);
	 * //if(entry==-1)return(-1); //t=entry*dim; j=0; switch(dimensions){ case
	 * 8: a[offset+(i++)]+=valueVector[entry][j++];//valuelist[t+(j++)]; case 7:
	 * a[offset+(i++)]+=valueVector[entry][j++];//valuelist[t+(j++)]; case 6:
	 * a[offset+(i++)]+=valueVector[entry][j++];//valuelist[t+(j++)]; case 5:
	 * a[offset+(i++)]+=valueVector[entry][j++];//valuelist[t+(j++)]; case 4:
	 * a[offset+(i++)]+=valueVector[entry][j++];//valuelist[t+(j++)]; case 3:
	 * a[offset+(i++)]+=valueVector[entry][j++];//valuelist[t+(j++)]; case 2:
	 * a[offset+(i++)]+=valueVector[entry][j++];//valuelist[t+(j++)]; case 1:
	 * a[offset+(i++)]+=valueVector[entry][j++];//valuelist[t+(j++)]; case 0:
	 * break; } } } }
	 */
