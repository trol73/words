		...
		for (int i = 0; i <= maximumClass; i++) {
			...
			if (classDimensions[i] > header.getCodeBooks().length
				...
				throw new VorbisFormatException(
						"There is a class dimension or class subclasses entry higher than the number of codebooks in the setup header.");
			}
...
//		int floorValues = 0;
	...
	protected Floor decodeFloor(VorbisStream vorbis, BitInputStream source)
		...
		for (int i = 0; i < partitionClassList.length; i++) {
			...
			if (cbits > 0) {
				cval = source
						.getInt(vorbis.getSetupHeader().getCodeBooks()[classMasterbooks[cls]]
								.getHuffmanRoot());
			...
			for (int j = 0; j < cdim; j++) {
				...
				cval >>>= cbits;
				if (book >= 0) {
					clone.yList[j + offset] = source.getInt(vorbis
							.getSetupHeader().getCodeBooks()[book]
							.getHuffmanRoot());
