 ...
 * serial number, chechsum and wether this page is the beginning or end of a
	...
	private static OggPage create(Object source, boolean skipData) throws IOException, EndOfOggStreamException, OggFormatException {
		...
		try {
			...
			if ( capture != 0x5367674f ) {
				// throw new
				// FormatException("Ogg page does not start with 'OggS' (0x4f676753)");
			...
			byte tmp = (byte) bdSource.getInt(8);		// header type
			boolean isContinuePage = (tmp & 1) != 0;	// 1 - is continued from the last page
			boolean isFirstPage = (tmp & 2) != 0;		// 2 - first page
			boolean isLastPage = (tmp & 4) != 0;		// 4 - last page of the stream
...
//			System.out.println("OggPage: "+tmp+" / "+streamSerialNumber+" / "+absoluteGranulePosition+" / "+pageSequenceNumber+" / "+pageSegments+" / "+totalLength);
...
//				System.out.println("createPage: "+absoluteGranulePosition*1000/44100);
			...
			return new OggPage(version, isContinuePage, isFirstPage, isLastPage, absoluteGranulePosition,
